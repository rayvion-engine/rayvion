...
public class MainMenuUI {
    ...
    public MainMenuUI(Runnable onPlayCallback) {
        ...
        table.add(playButton).width(300).height(60).row();
    ...
    public void render(float delta) {
        ...
        Gdx.gl.glClearColor(0.05f, 0.0f, 0.1f, 1);
        ...
        for (float y = offset; y < height; y += lineSpacing) {
            ...
            shapeRenderer.setColor(0, 1, 1, alpha); // Cyan
            shapeRenderer.line(0, y, width, y);
        }

        // Vertical lines radiating from a point
        float centerX = width / 2f;
        for (int i = -15; i <= 15; i++) {
            float bottomX = centerX + (i * 100f);
            shapeRenderer.setColor(1, 0, 1, 0.5f); // Magenta
            shapeRenderer.line(centerX, height * 1.2f, bottomX, 0);
        }
